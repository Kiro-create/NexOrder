...
/**
 * Concrete Template Method implementation for inventory reports.
 * Uses Strategy (formatting) + Bridge (output) through {@link ReportStrategyProvider}.
 */
public class InventoryReportGeneration extends ReportGenerationTemplate {
    ...
    protected boolean validateParameters(Map<String, Object> parameters) {
        log.info("InventoryReportGeneration: validation started");
        if (!parameters.containsKey("formatChoice")) {
            log.error("InventoryReportGeneration: Missing parameter formatChoice");
            return false;
        }
        int formatChoice = (int) parameters.get("formatChoice");
        if (formatChoice != PDF && formatChoice != EXCEL) {
            log.error("InventoryReportGeneration: Invalid formatChoice=" + formatChoice);
            return false;
        }
        log.info("InventoryReportGeneration: validation passed");
        return true;
    }

    @Override
    protected Object collectData(Map<String, Object> parameters) {
        log.info("InventoryReportGeneration: collectData started");
        Object productsObj = parameters.get("products");
        if (!(productsObj instanceof List)) {
            log.error("InventoryReportGeneration: products parameter missing/invalid");
            return null;
        }
        ...
        int formatChoice = (int) parameters.get("formatChoice");
        String data = buildData(products, INVENTORY, formatChoice);
        log.info("InventoryReportGeneration: collectData completed (chars=" + data.length() + ")");
        return data;
    }

    @Override
    protected Object formatData(Object data, Map<String, Object> parameters) {
        log.info("InventoryReportGeneration: formatting started");
        InputValidator.validateNotNull(data, "data");
        if (!(data instanceof String)) {
            log.error("InventoryReportGeneration: Invalid data type for formatting");
            return null;
        }
        int formatChoice = (int) parameters.get("formatChoice");
        ReportFormattingStrategy strategy = ReportStrategyProvider.getStrategy(INVENTORY, formatChoice);
        log.info("InventoryReportGeneration: formatting strategy selected");
        return new FormattedPayload((String) data, strategy);
    }

    @Override
    protected boolean outputReport(Object formattedData) {
        log.info("InventoryReportGeneration: output started");
        if (!(formattedData instanceof FormattedPayload)) {
            log.error("InventoryReportGeneration: Invalid formatted payload type");
            return false;
        }
        ...
        log.info("InventoryReportGeneration: output completed");
        return true;
    }
